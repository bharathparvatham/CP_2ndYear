...
def helperBinarySearchValues(L, v, start, end, listOfTuples):
    if start > end:
        return listOfTuples

    mid = (start + end) // 2

    listOfTuples.append((mid, L[mid]))

    if L[mid] == v:
        return listOfTuples

    elif L[mid] < v:
        return helperBinarySearchValues(L, v, mid + 1, end, listOfTuples)
    else:
        return helperBinarySearchValues(L, v, start, mid - 1, listOfTuples)


def recursion_binarysearchvalues(L, v):
    # Your codes goes here
    return helperBinarySearchValues(L, v, 0, len(L) - 1, [])
